...
//			System.out.println("Plain text hashed password:" + hashed_password);					
		...
		this.userId = KeyFactory.keyToString(speaker.getKey());
		
		if (speaker.getToken().isEmpty()){
			speaker.setToken(Authentication.generateToken(speaker.getEmail() + ":" + speaker.getPassword()));
			DaoFactory.speakerDao().updateSpeaker(speaker);
		}
		
		this.token = speaker.getToken();
